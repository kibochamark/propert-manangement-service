"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var ChargeRepository_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ChargeRepository = void 0;
const common_1 = require("@nestjs/common");
const prismaservice_service_1 = require("../prismaservice/prismaservice.service");
let ChargeRepository = ChargeRepository_1 = class ChargeRepository {
    prismaService;
    logger = new common_1.Logger(ChargeRepository_1.name);
    constructor(prismaService) {
        this.prismaService = prismaService;
    }
    async create(input, createdById) {
        this.logger.log(`Creating ${input.type} charge of ${input.amount} for tenancy ${input.tenancyId}`);
        return this.prismaService.charge.create({
            data: { ...input, createdById },
        });
    }
    async findById(id) {
        return this.prismaService.charge.findUnique({ where: { id } });
    }
    async findByTenancy(tenancyId, type) {
        return this.prismaService.charge.findMany({
            where: { tenancyId, type },
            orderBy: [{ periodMonth: 'asc' }, { createdAt: 'asc' }],
        });
    }
    async voidCharge(id, reason, voidedById) {
        return this.prismaService.charge.update({
            where: { id },
            data: { voidedAt: new Date(), voidReason: reason, voidedById },
        });
    }
};
exports.ChargeRepository = ChargeRepository;
exports.ChargeRepository = ChargeRepository = ChargeRepository_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prismaservice_service_1.PrismaService])
], ChargeRepository);
//# sourceMappingURL=charge.repository.js.map